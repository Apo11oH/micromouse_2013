...
void setup(){
  ...
  // Setup on board button
  //pinMode(BOARD_BUTTON_PIN, INPUT);
  //attachInterrupt(BOARD_BUTTON_PIN, buttonHandler, RISING);
...
void setSeen(int curLocX, int curLocY, int setDirec, int traversal[][MAZE_MAX_X]){
  switch(setDirec){
    // Set North wall
    case 0: traversal[curLocY][curLocX] |= 0x10; 
            traversal[curLocY][curLocX] &= 0xfe;
            if( curLocY>0 ){
                traversal[curLocY-1][curLocX] |= 0x40;
                traversal[curLocY-1][curLocX] &= 0xfb;
            }
            break;
    // Set East wall
    case 1: traversal[curLocY][curLocX] |= 0x20;
            traversal[curLocY][curLocX] &= 0xfd; 
            if( curLocX<MAZE_MAX_X-1 ){
                traversal[curLocY][curLocX+1] |= 0x80;
                traversal[curLocY][curLocX+1] &= 0xf7;
            }
            break;
    // Set South wall
    case 2: traversal[curLocY][curLocX] |= 0x40;
            traversal[curLocY][curLocX] &= 0xfb; 
            if( curLocY<MAZE_MAX_Y-1 ){
                traversal[curLocY+1][curLocX] |= 0x10;
                traversal[curLocY+1][curLocX] &= 0xfe;
            }
            break;
    // Set West wall
    case 3: traversal[curLocY][curLocX] |= 0x80;
            traversal[curLocY][curLocX] &= 0xf7; 
            if( curLocX>0 ){
                traversal[curLocY][curLocX-1] |= 0x10;
                traversal[curLocY][curLocX-1] &= 0xfe;
            }
            break;
  }
}
...
void setTraversalMap(int curLocX, int curLocY, int curDirec, int traversal[][MAZE_MAX_X]){
  ...
  if( analogRead(frontLongSensor) > 1900 ){
    //SerialUSB.println("Wall front");
    setWall( curLocX, curLocY, front, traversal);
  }else{
    //SerialUSB.println("No Wall front");
    setSeen( curLocX, curLocY, front, traversal);
  }
  // Check right sensor
  if( analogRead(sensorFR) > 2200 ){
    ...
    //SerialUSB.println("No Wall right");
    setSeen( curLocX, curLocY, right, traversal);
  }
  // Check left sensor
  if( analogRead(sensorFL) > 2200 ){
    ...
    //SerialUSB.println("No Wall left");
    setSeen( curLocX, curLocY, left, traversal);
  }
  //delay(1000);
}
...
int findNextDirection(int currentX, int currentY){
  ...
  // check front 
  if( currentY > 0 ){
      if( (traversalMap[currentY][currentX]&0x01) != 0x01 ){
          neighborhood[0] = potential[currentY-1][currentX];
      }
  }
  // check right
  if( currentX < MAZE_MAX_X - 1 ){
      if( (traversalMap[currentY][currentX]&0x02) != 0x02 ){
          neighborhood[1] = potential[currentY][currentX+1];
      }
  }
  // check back
  if( currentY < MAZE_MAX_Y - 1 ){
      if( (traversalMap[currentY][currentX]&0x04) != 0x04 ){
          neighborhood[2] = potential[currentY+1][currentX];
      }
  }
  // check left
  if( currentX > 0 ){
      if( (traversalMap[currentY][currentX]&0x08) != 0x08 ){
          neighborhood[3] = potential[currentY][currentX-1];
      }
  }
  
  lowestPotential = neighborhood[0];
  lowestIndex = 0;
  for(i=1; i<4; i++){
    if(neighborhood[i] < lowestPotential){
      lowestPotential = neighborhood[i];
      lowestIndex = i;
    }
  }
